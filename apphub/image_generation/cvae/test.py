...
class SavedImagePair(Trace):
    ...
    def on_batch_end(self, data: Data):
        ...
        if self.system.batch_idx == 0:
            for i in range(10):
                img = np.squeeze(input_img_np[i], axis=2)
                img = (img * 255).astype(np.uint8)
                img = Image.fromarray(img)
                img.save("input_img_np_{}.png".format(i))

                img = np.squeeze(decode_img_np[i], axis=2)
                img = (img * 255).astype(np.uint8)
                img = Image.fromarray(img)
                img.save("decode_img_np_{}.png".format(i))
...
def get_estimator(batch_size=100, epochs=100, steps_per_epoch=None, model_dir=tempfile.mkdtemp()):
    ...
    encode_model = fe.build(model_fn=encoder_net, optimizer_fn=lambda: tf.optimizers.Adam(lr=1e-4))
    decode_model = fe.build(model_fn=decoder_net, optimizer_fn=lambda: tf.optimizers.Adam(lr=1e-4))
