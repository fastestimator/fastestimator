...
class CutMixBatch(TensorOp):
    ...
    def forward(self, data: List[Tensor], state: Dict[str, Any]) -> Tuple[Tensor, Tensor]:
        ...
        if tf.is_tensor(x):
            flipped_x = flip(x, [0])
            patches = flipped_x[:, bbox_y1:bbox_y2, bbox_x1:bbox_x2, :] - x[:, bbox_y1:bbox_y2, bbox_x1:bbox_x2, :]
            ...
            flipped_x = flip(x, [0])
            x[:, :, bbox_y1:bbox_y2, bbox_x1:bbox_x2] = flipped_x[:, :, bbox_y1:bbox_y2, bbox_x1:bbox_x2]
        # adjust lambda to match pixel ratio
        lam = 1 - cast(((bbox_x2 - bbox_x1) * (bbox_y2 - bbox_y1)), dtype="float32") / cast((width * height), "float32")
        flipped_y = flip(y, [0]) * (1. - lam)
        mixed_y = (y * lam) + flipped_y
        return x, mixed_y
