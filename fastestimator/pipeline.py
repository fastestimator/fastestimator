...
from typing import Any, Dict, Iterable, List, Optional, Set, Tuple, Type, TypeVar, Union, cast
...
class Pipeline:
    ...
    def __call__(self,
        ...
        if isinstance(dataset, InterleaveDataset):
            ...
            self.ctx_ops, self.ctx_batch_info, self.ctx_batch_ops = [], [], []
            for tag in dataset.tags:
                ctx_ops, ctx_batch_info, ctx_batch_ops = self._get_op_split(mode=mode, epoch=epoch, ds_id={ds_id, tag} if isinstance(tag, str) else ds_id)
                self.ctx_ops.append(ctx_ops)
                self.ctx_batch_info.append(ctx_batch_info)
                self.ctx_batch_ops.append(ctx_batch_ops)
            # Decide on the batch size (this might still be ignored later if the user is using a BatchDataset)
            self.ctx_batch_size = [ctx_batch_info.batch_size for ctx_batch_info in self.ctx_batch_info]
