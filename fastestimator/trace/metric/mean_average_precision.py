...
class MeanAvgPrecision(Trace):
    ...
    Returns:
    """
    def __init__(self, num_classes: int, pred_key: str = 'pred', gt_key = 'bbox', mode: str = "eval",
    ...
    def _get_ids_in_epoch(self, idx_in_batch):
    ...
    def on_batch_end(self, data: Data):
        ...
        for gt_item in gt:
            ...
            id_epoch = self._get_ids_in_epoch(idx_in_batch)
    ...
    def accumulate(self):
        ...
        iou_thresh = len(self.iou_thres)
        recall_thresh = len(self.recall_thres)
        ...
        precision = -np.ones((iou_thresh, recall_thresh, K))
        recall = -np.ones((iou_thresh, K))
        scores = -np.ones((iou_thresh, recall_thresh, K))
        ...
        for k in cat_list_zeroidx:
            ...
            for t, (tp, fp) in enumerate(zip(tp_sum, fp_sum)):
                ...
                q = np.zeros((recall_thresh, ))
                ss = np.zeros((recall_thresh, ))
                ...
                for i in range(nd - 1, 0, -1):
                    if pr[i] > pr[i - 1]:
                        pr[i - 1] = pr[i]
                inds = np.searchsorted(rc, self.recall_thres, side='left')
                try:
        ...
        self.eval = {
            'counts': [iou_thresh, recall_thresh, K],
            'precision': precision,
            'recall': recall,
            'scores': scores, }
    ...
    def evaluate_img(self, cat_id, img_id):
        ...
        dt = [dt[i] for i in dtind[0:self.max_detection]]
    ...
    def compute_iou(self, dt, gt):
        ...
        if len(dt) > self.max_detection:
            dt = dt[0:self.max_detection]
