...
class TestParser(TestCase):
        ...
        with patch('fastestimator.visualization.parse_logs.open', parse_mock):
            with patch('fastestimator.visualization.parse_logs.graph_metrics', graph_mock):
                parse_files([TestParser.mock_good_file_path, TestParser.mock_good_file_path_2], log_extension='.txt',
                            smooth_factor=1.5,
                            save=True, save_path='/out/', ignore_metrics=None, share_legend=False,
                            pretty_names=True)
    ...
    def test_graph_metrics_save(self):
        ...
        fig_mock = MagicMock()
        dir_mock = MagicMock()
        with patch('fastestimator.visualization.parse_logs.plt.savefig', fig_mock), patch(
                'fastestimator.visualization.parse_logs.os.makedirs', dir_mock):
