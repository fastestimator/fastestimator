...
from fastestimator.record.preprocess import ImageReader, Resize, NumpyPreprocess
from fastestimator.network.model import ModelOp, build
from fastestimator.network.loss import Loss

class String2List(NumpyPreprocess):
...
class RelativeCoordinate(NumpyPreprocess):
    def forward(self, data):
...
class GenerateTarget(NumpyPreprocess):
...
def get_estimator():
    ...
    writer = RecordWriter(train_data=train_csv,
                        ...
                        ops=[ImageReader(inputs="image", parent_path=path, outputs="image"), 
                            ...
                            RelativeCoordinate(inputs=("image", "x1", "y1", "x2", "y2"), outputs=("image", "x1", "y1", "x2", "y2")),
