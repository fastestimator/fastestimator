...
from fastestimator.estimator.trace import Accuracy, Dice
from fastestimator.estimator.estimator import Estimator
import tensorflow as tf
from tensorflow.keras.layers import Input, Conv2D, Dropout, MaxPool2D, UpSampling2D, concatenate, Conv2DTranspose
...
def get_estimator():
    ...
    pipeline = Pipeline(batch_size=8, data=writer, ops=[Minmax(inputs="image",outputs="image"),Minmax(inputs="mask", outputs="mask"),
    ...
    estimator = Estimator(network=network, pipeline=pipeline, epochs=25, log_steps=20, traces=[Dice("mask","pred_segment")])
