...
class TestLRScheduler(unittest.TestCase):
    ...
    def test_torch_model_arc_train_eval(self):
        model_torch = fe.build(model_fn=LeNet_torch, optimizer_fn="adam")
        lr_before = get_lr(model=model_torch)
        estimator = self.create_estimator_for_arc(model_torch, use_eval=True, axis=0)
        estimator.fit()
        lr_after = get_lr(model=model_torch)
    ...
    def test_torch_model_arc_train_only(self):
        model_torch = fe.build(model_fn=LeNet_torch, optimizer_fn="adam")
        lr_before = get_lr(model=model_torch)
        estimator = self.create_estimator_for_arc(model_torch, use_eval=False, axis=0)
        estimator.fit()
        lr_after = get_lr(model=model_torch)
        lr_ratio = lr_after / lr_before
        increased = math.isclose(lr_ratio, 1.618, rel_tol=1e-5)
