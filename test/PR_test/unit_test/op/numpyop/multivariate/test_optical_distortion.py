...
from fastestimator.op.numpyop.multivariate import OpticalDistortion
...
class TestOpticalDistortion(unittest.TestCase):
    ...
    def test_input(self):
        optical_distortion = OpticalDistortion(image_in='x')
        output = optical_distortion.forward(data=self.single_input, state={})
    ...
    def test_input_image_and_mask(self):
        optical_distortion = OpticalDistortion(image_in='x', mask_in='x_mask')
        output = optical_distortion.forward(data=self.input_image_and_mask, state={})
        with self.subTest('Check output type'):
            self.assertEqual(type(output), list)
        with self.subTest('Check output image shape'):
