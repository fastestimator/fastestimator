...
from fastestimator.op.numpyop.multivariate import RandomFlip3D
...
class TestRandomFlip3D(unittest.TestCase):
    ...
    def test_input(self):
        op = RandomFlip3D(inputs='x', outputs='x')
    ...
    def test_input_image_and_mask(self):
        op = RandomFlip3D(inputs=('x', 'mask'), outputs=('x', 'mask'))
    ...
    def test_4d_input(self):
        data = [np.random.rand(16, 28, 28, 3).astype(np.float32)]
        op = RandomFlip3D(inputs='x', outputs='x')
        output = op.forward(data=data, state={})
        self.assertEqual(output[0].shape, (16, 28, 28, 3))
    ...
    def test_specific_axes(self):
        op = RandomFlip3D(inputs='x', outputs='x', spatial_axes=(0, ))
        output = op.forward(data=self.single_input, state={})
        self.assertEqual(output[0].shape, self.single_output_shape)
