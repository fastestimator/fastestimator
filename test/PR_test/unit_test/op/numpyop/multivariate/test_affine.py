...
from fastestimator.op.numpyop.multivariate import Affine
...
class TestAffine(unittest.TestCase):
    ...
    def test_single_input(self):
        affine = Affine(image_in='x')
        output = affine.forward(data=self.single_input, state={})
    ...
    def test_input_image_and_mask(self):
        affine = Affine(image_in='x', mask_in='x_mask')
        output = affine.forward(data=self.input_image_and_mask, state={})
        with self.subTest('Check output type'):
            self.assertEqual(type(output), list)
        with self.subTest('Check output image shape'):
