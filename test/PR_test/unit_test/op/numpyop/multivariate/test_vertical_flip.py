...
from fastestimator.op.numpyop.multivariate import VerticalFlip
...
class TestVerticalFlip(unittest.TestCase):
    ...
    def test_input(self):
        vertical_flip = VerticalFlip(image_in='x')
        output = vertical_flip.forward(data=self.single_input, state={})
    ...
    def test_input_image_and_mask(self):
        vertical_flip = VerticalFlip(image_in='x', mask_in='x_mask')
        output = vertical_flip.forward(data=self.input_image_and_mask, state={})
        with self.subTest('Check output type'):
            self.assertEqual(type(output), list)
        with self.subTest('Check output image shape'):
