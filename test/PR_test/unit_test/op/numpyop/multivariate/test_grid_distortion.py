...
from fastestimator.op.numpyop.multivariate import GridDistortion
...
class TestGridDistortion(unittest.TestCase):
    ...
    def test_input(self):
        distortion = GridDistortion(image_in='x')
        output = distortion.forward(data=self.single_input, state={})
    ...
    def test_input_image_and_mask(self):
        distortion = GridDistortion(image_in='x', mask_in='x_mask')
        output = distortion.forward(data=self.input_image_and_mask, state={})
        with self.subTest('Check output type'):
            self.assertEqual(type(output), list)
        with self.subTest('Check output image shape'):
