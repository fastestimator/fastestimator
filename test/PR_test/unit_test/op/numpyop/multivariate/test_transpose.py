...
from fastestimator.op.numpyop.multivariate import Transpose
...
class TestTranspose(unittest.TestCase):
    ...
    def test_input(self):
        transpose = Transpose(image_in='x')
        output = transpose.forward(data=self.single_input, state={})
    ...
    def test_input_image_and_mask(self):
        transpose = Transpose(image_in='x', mask_in='x_mask')
        output = transpose.forward(data=self.input_image_and_mask, state={})
        with self.subTest('Check output type'):
            self.assertEqual(type(output), list)
        with self.subTest('Check output image shape'):
