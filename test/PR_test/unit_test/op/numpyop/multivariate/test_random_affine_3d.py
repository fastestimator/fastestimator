...
from fastestimator.op.numpyop.multivariate import RandomAffine3D


class TestRandomAffine3D(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_input(self):
        op = RandomAffine3D(inputs='x', outputs='x', rotate_range=10.0, scale_range=0.1, translate_range=5.0)
    ...
    def test_input_image_and_mask(self):
        op = RandomAffine3D(inputs=('x', 'mask'), outputs=('x', 'mask'), rotate_range=10.0, label_keys='mask')
    ...
    def test_4d_input(self):
        ...
        op = RandomAffine3D(inputs='x', outputs='x', rotate_range=10.0)
    ...
    def test_per_axis_rotation(self):
        op = RandomAffine3D(inputs='x', outputs='x', rotate_range=[5.0, 10.0, 15.0])
        output = op.forward(data=self.single_input, state={})
        self.assertEqual(output[0].shape, self.single_output_shape)
