...
from fastestimator.op.numpyop.multivariate import RandomGridShuffle


class TestRandomGridShuffle(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_input(self):
        grid_shuffle = RandomGridShuffle(image_in='x')
        output = grid_shuffle.forward(data=self.single_input, state={})
    ...
    def test_input_image_and_mask(self):
        grid_shuffle = RandomGridShuffle(image_in='x', mask_in='x_mask')
        output = grid_shuffle.forward(data=self.input_image_and_mask, state={})
        with self.subTest('Check output type'):
            self.assertEqual(type(output), list)
        with self.subTest('Check output image shape'):
