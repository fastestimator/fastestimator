...
from fastestimator.op.numpyop.multivariate import RandomRotate3D


class TestRandomRotate3D(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_input(self):
        op = RandomRotate3D(inputs='x', outputs='x', angle_range=10.0)
    ...
    def test_input_image_and_mask(self):
        op = RandomRotate3D(inputs=('x', 'mask'), outputs=('x', 'mask'), angle_range=10.0, label_keys='mask')
