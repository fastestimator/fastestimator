...
from fastestimator.op.numpyop.univariate import HueSaturationValue


class TestHueSaturationValue(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_single_input(self):
        hue_saturation = HueSaturationValue(inputs='x', outputs='x')
        output = hue_saturation.forward(data=self.single_input, state={})
    ...
    def test_multi_input(self):
        hue_saturation = HueSaturationValue(inputs='x', outputs='x')
        output = hue_saturation.forward(data=self.multi_input, state={})
        with self.subTest('Check output type'):
            self.assertEqual(type(output), list)
