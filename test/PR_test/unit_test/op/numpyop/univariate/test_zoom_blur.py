...
from fastestimator.op.numpyop.univariate import ZoomBlur
...
class TestZoomBlur(unittest.TestCase):
    ...
    def test_single_input(self):
        ...
        output = zblur.forward(data=self.single_input, state={})
    ...
    def test_input_image_and_mask(self):
        zblur = ZoomBlur(inputs='x', outputs='x')
        output = zblur.forward(data=self.multi_input, state={})
        with self.subTest('Check output type'):
            self.assertEqual(type(output), list)
