...
from fastestimator.op.numpyop.univariate import HistogramMatching
...
class TestHistogramMatching(unittest.TestCase):
    ...
    def test_single_input(self):
        hist = HistogramMatching(inputs='x', outputs='x', reference_images=self.ref_image_single)
        output = hist.forward(data=self.single_input, state={})
    ...
    def test_input_image_and_mask(self):
        hist = HistogramMatching(inputs='x', outputs='x', reference_images=self.ref_image_multiple)
        output = hist.forward(data=self.single_input, state={})
        with self.subTest('Check output type'):
            self.assertEqual(type(output), list)
        with self.subTest('Check output image shape'):
