...
from fastestimator.op.numpyop.univariate import MultiplicativeNoise


class TestMultiplicativeNoise(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_single_input(self):
        mult_noise = MultiplicativeNoise(inputs='x', outputs='x')
        output = mult_noise.forward(data=self.single_input, state={})
    ...
    def test_multi_input(self):
        mult_noise = MultiplicativeNoise(inputs='x', outputs='x')
        output = mult_noise.forward(data=self.multi_input, state={})
        with self.subTest('Check output type'):
            self.assertEqual(type(output), list)
