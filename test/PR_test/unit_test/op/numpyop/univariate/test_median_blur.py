...
from fastestimator.op.numpyop.univariate import MedianBlur
...
class TestMedianBlur(unittest.TestCase):
    ...
    def test_single_input(self):
        median_blur = MedianBlur(inputs='x', outputs='x')
        output = median_blur.forward(data=self.single_input, state={})
    ...
    def test_multi_input(self):
        median_blur = MedianBlur(inputs='x', outputs='x')
        output = median_blur.forward(data=self.multi_input, state={})
        with self.subTest('Check output type'):
            self.assertEqual(type(output), list)
