...
from fastestimator.op.numpyop.univariate import ShearX
...
class TestShearX(unittest.TestCase):
    ...
    def test_single_input(self):
        shear_x = ShearX(inputs='x', outputs='x')
        output = shear_x.forward(data=self.single_input, state={})
    ...
    def test_multi_input(self):
        shear_x = ShearX(inputs='x', outputs='x')
        output = shear_x.forward(data=self.multi_input, state={})
        with self.subTest('Check output type'):
            self.assertEqual(type(output), list)
