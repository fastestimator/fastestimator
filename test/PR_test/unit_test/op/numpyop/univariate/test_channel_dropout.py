...
from fastestimator.op.numpyop.univariate import ChannelDropout


class TestChannelDropout(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_single_input(self):
        channel_dropout = ChannelDropout(inputs='x', outputs='x')
        output = channel_dropout.forward(data=self.single_input, state={})
    ...
    def test_input_image_and_mask(self):
        channel_dropout = ChannelDropout(inputs='x', outputs='x')
        output = channel_dropout.forward(data=self.multi_input, state={})
        with self.subTest('Check output type'):
            self.assertEqual(type(output), list)
