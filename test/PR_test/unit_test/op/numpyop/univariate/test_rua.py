...
from fastestimator.op.numpyop.univariate import RUA
...
class TestRUA(unittest.TestCase):
    ...
    def test_single_input(self):
        rua = RUA(inputs='x', outputs='x')
        output = rua.forward(data=self.single_input, state={})
    ...
    def test_multi_input(self):
        rua = RUA(inputs=['x', 'y'], outputs=['x', 'y'])
        output = rua.forward(data=self.multi_input, state={})
        with self.subTest('Check output type'):
            self.assertEqual(type(output), list)
