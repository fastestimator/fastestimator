...
from fastestimator.op.numpyop.univariate import Equalize
...
class TestEqualize(unittest.TestCase):
    ...
    def test_single_input(self):
        equalize = Equalize(inputs='x', outputs='x')
        output = equalize.forward(data=self.single_input, state={})
    ...
    def test_multi_input(self):
        equalize = Equalize(inputs='x', outputs='x')
        output = equalize.forward(data=self.multi_input, state={})
        with self.subTest('Check output type'):
            self.assertEqual(type(output), list)
