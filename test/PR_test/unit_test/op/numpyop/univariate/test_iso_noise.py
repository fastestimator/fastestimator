...
from fastestimator.op.numpyop.univariate import ISONoise


class TestISONoise(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_single_input(self):
        isonoise = ISONoise(inputs='x', outputs='x')
        output = isonoise.forward(data=self.single_input, state={})
    ...
    def test_multi_input(self):
        isonoise = ISONoise(inputs='x', outputs='x')
        output = isonoise.forward(data=self.multi_input, state={})
        with self.subTest('Check output type'):
            self.assertEqual(type(output), list)
