...
from fastestimator.op.numpyop.univariate import TranslateX
...
class TestTranslateX(unittest.TestCase):
    ...
    def test_single_input(self):
        translate_x = TranslateX(inputs='x', outputs='x')
        output = translate_x.forward(data=self.single_input, state={})
    ...
    def test_multi_input(self):
        translate_x = TranslateX(inputs='x', outputs='x')
        output = translate_x.forward(data=self.multi_input, state={})
        with self.subTest('Check output type'):
            self.assertEqual(type(output), list)
