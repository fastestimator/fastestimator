...
from fastestimator.op.numpyop.univariate import Sharpness
...
class TestSharpness(unittest.TestCase):
    ...
    def test_single_input(self):
        ...
        output = sharpness.forward(data=self.single_input, state={})
    ...
    def test_multi_input(self):
        sharpness = Sharpness(inputs='x', outputs='x')
        output = sharpness.forward(data=self.multi_input, state={})
        with self.subTest('Check output type'):
            self.assertEqual(type(output), list)
