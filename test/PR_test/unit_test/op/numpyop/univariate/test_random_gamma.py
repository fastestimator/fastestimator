...
from fastestimator.op.numpyop.univariate import RandomGamma
...
class TestRandomGamma(unittest.TestCase):
    ...
    def test_single_input(self):
        random_gamma = RandomGamma(inputs='x', outputs='x')
        output = random_gamma.forward(data=self.single_input, state={})
    ...
    def test_multi_input(self):
        random_gamma = RandomGamma(inputs='x', outputs='x')
        output = random_gamma.forward(data=self.multi_input, state={})
        with self.subTest('Check output type'):
            self.assertEqual(type(output), list)
