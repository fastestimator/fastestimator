...
from fastestimator.op.numpyop.univariate import RandomRain


class TestRandomRain(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_single_input(self):
        random_rain = RandomRain(inputs='x', outputs='x')
        output = random_rain.forward(data=self.single_input, state={})
    ...
    def test_multi_input(self):
        random_rain = RandomRain(inputs='x', outputs='x')
        output = random_rain.forward(data=self.multi_input, state={})
        with self.subTest('Check output type'):
            self.assertEqual(type(output), list)
