...
from fastestimator.op.numpyop.univariate import FromFloat
...
class TestFromFloat(unittest.TestCase):
    ...
    def test_single_input(self):
        fromfloat = FromFloat(inputs='x', outputs='x')
        output = fromfloat.forward(data=self.single_input, state={})
    ...
    def test_multi_input(self):
        fromfloat = FromFloat(inputs='x', outputs='x')
        output = fromfloat.forward(data=self.multi_input, state={})
        with self.subTest('Check output type'):
            self.assertEqual(type(output), list)
