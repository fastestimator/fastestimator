...
from fastestimator.op.numpyop.univariate import RGBShift
...
class TestRGBShift(unittest.TestCase):
    ...
    def test_single_input(self):
        rgb_shift = RGBShift(inputs='x', outputs='x')
        output = rgb_shift.forward(data=self.single_input, state={})
    ...
    def test_multi_input(self):
        rgb_shift = RGBShift(inputs='x', outputs='x')
        output = rgb_shift.forward(data=self.multi_input, state={})
        with self.subTest('Check output type'):
            self.assertEqual(type(output), list)
