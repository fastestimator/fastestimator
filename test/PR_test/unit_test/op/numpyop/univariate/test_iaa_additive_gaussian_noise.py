...
from fastestimator.op.numpyop.univariate import IAAAdditiveGaussianNoise
...
class TestGaussianNoise(unittest.TestCase):
    ...
    def test_single_input(self):
        gaussian_noise = IAAAdditiveGaussianNoise(inputs='x', outputs='x')
        output = gaussian_noise.forward(data=self.single_input, state={})
    ...
    def test_multi_input(self):
        gaussian_noise = IAAAdditiveGaussianNoise(inputs='x', outputs='x')
        output = gaussian_noise.forward(data=self.multi_input, state={})
        with self.subTest('Check output type'):
            self.assertEqual(type(output), list)
