...
from fastestimator.op.numpyop.univariate import MotionBlur
...
class TestMotionBlur(unittest.TestCase):
    ...
    def test_single_input(self):
        motion_blur = MotionBlur(inputs='x', outputs='x')
        output = motion_blur.forward(data=self.single_input, state={})
    ...
    def test_multi_input(self):
        motion_blur = MotionBlur(inputs='x', outputs='x')
        output = motion_blur.forward(data=self.multi_input, state={})
        with self.subTest('Check output type'):
            self.assertEqual(type(output), list)
