...
from fastestimator.op.numpyop.univariate import ChannelShuffle
...
class TestChannelShuffle(unittest.TestCase):
    ...
    def test_single_input(self):
        channel_shuffle = ChannelShuffle(inputs='x', outputs='x')
        output = channel_shuffle.forward(data=self.single_input, state={})
    ...
    def test_input_image_and_mask(self):
        channel_shuffle = ChannelShuffle(inputs='x', outputs='x')
        output = channel_shuffle.forward(data=self.multi_input, state={})
        with self.subTest('Check output type'):
            self.assertEqual(type(output), list)
