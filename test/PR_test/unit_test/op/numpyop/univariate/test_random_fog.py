...
from fastestimator.op.numpyop.univariate import RandomFog


class TestRandomFog(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_single_input(self):
        random_fog = RandomFog(inputs='x', outputs='x')
        output = random_fog.forward(data=self.single_input, state={})
    ...
    def test_multi_input(self):
        random_fog = RandomFog(inputs='x', outputs='x')
        output = random_fog.forward(data=self.multi_input, state={})
        with self.subTest('Check output type'):
            self.assertEqual(type(output), list)
