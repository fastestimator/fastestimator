...
from fastestimator.op.numpyop.univariate import Downscale
...
class TestDownscale(unittest.TestCase):
    ...
    def test_single_input(self):
        downscale = Downscale(inputs='x', outputs='x')
        output = downscale.forward(data=self.single_input, state={})
    ...
    def test_multi_input(self):
        downscale = Downscale(inputs='x', outputs='x')
        output = downscale.forward(data=self.multi_input, state={})
        with self.subTest('Check output type'):
            self.assertEqual(type(output), list)
