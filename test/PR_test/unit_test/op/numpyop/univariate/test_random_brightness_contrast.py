...
from fastestimator.op.numpyop.univariate import RandomBrightnessContrast
...
class TestRandomBrightnessContrast(unittest.TestCase):
    ...
    def test_single_input(self):
        random_brightness_contrast = RandomBrightnessContrast(inputs='x', outputs='x')
        output = random_brightness_contrast.forward(data=self.single_input, state={})
    ...
    def test_multi_input(self):
        random_brightness_contrast = RandomBrightnessContrast(inputs='x', outputs='x')
        output = random_brightness_contrast.forward(data=self.multi_input, state={})
        with self.subTest('Check output type'):
            self.assertEqual(type(output), list)
