...
from fastestimator.op.numpyop.univariate import Defocus


class TestDefocus(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_single_input(self):
        defocus = Defocus(inputs='x', outputs='x')
        output = defocus.forward(data=self.single_input, state={})
    ...
    def test_input_image_and_mask(self):
        defocus = Defocus(inputs='x', outputs='x')
        output = defocus.forward(data=self.multi_input, state={})
        with self.subTest('Check output type'):
            self.assertEqual(type(output), list)
