...
from fastestimator.op.numpyop.univariate import Solarize
...
class TestSolarize(unittest.TestCase):
    ...
    def test_single_input(self):
        solarize = Solarize(inputs='x', outputs='x')
        output = solarize.forward(data=self.single_input, state={})
    ...
    def test_multi_input(self):
        solarize = Solarize(inputs='x', outputs='x')
        output = solarize.forward(data=self.multi_input, state={})
        with self.subTest('Check output type'):
            self.assertEqual(type(output), list)
