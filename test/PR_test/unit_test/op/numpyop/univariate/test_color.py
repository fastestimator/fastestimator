...
from fastestimator.op.numpyop.univariate import Color
...
class TestColor(unittest.TestCase):
    ...
    def test_single_input(self):
        color = Color(inputs='x', outputs='x')
        output = color.forward(data=self.single_input, state={})
    ...
    def test_multi_input(self):
        color = Color(inputs='x', outputs='x')
        output = color.forward(data=self.multi_input, state={})
        with self.subTest('Check output type'):
            self.assertEqual(type(output), list)
    ...
    def test_invalid_dtype(self):
        ...
        with self.assertRaises(ValueError):
            color.forward(data=invalid_input, state={})
