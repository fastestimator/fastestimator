...
from fastestimator.op.numpyop.univariate import ColorJitter
...
class TestColorJitter(unittest.TestCase):
    ...
    def test_single_input(self):
        color_jitter = ColorJitter(inputs='x', outputs='x')
        output = color_jitter.forward(data=self.single_input, state={})
    ...
    def test_multi_input(self):
        color_jitter = ColorJitter(inputs='x', outputs='x')
        output = color_jitter.forward(data=self.multi_input, state={})
        with self.subTest('Check output type'):
            self.assertEqual(type(output), list)
