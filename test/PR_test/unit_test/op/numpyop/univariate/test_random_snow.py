...
from fastestimator.op.numpyop.univariate import RandomSnow


class TestRandomSnow(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_single_input(self):
        random_snow = RandomSnow(inputs='x', outputs='x')
        output = random_snow.forward(data=self.single_input, state={})
    ...
    def test_multi_input(self):
        random_snow = RandomSnow(inputs='x', outputs='x')
        output = random_snow.forward(data=self.multi_input, state={})
        with self.subTest('Check output type'):
            self.assertEqual(type(output), list)
