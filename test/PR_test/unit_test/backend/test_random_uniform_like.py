...
class TestRandomUniformLike(unittest.TestCase):
    ...
    def test_random_uniform_np_type(self):
        self.assertIsInstance(random_uniform_like(self.test_np), np.ndarray, 'Output must be NumPy Array')
    ...
    def test_random_uniform_torch_value(self):
        output_shape = (2, 2)
        self.assertTrue((random_uniform_like(self.test_torch).size() == output_shape),
                        'Output tensor shape should be same as input')
