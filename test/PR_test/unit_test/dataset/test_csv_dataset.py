...
class TestCSVDataset(unittest.TestCase):
    ...
    def _make_csv() -> str:
        ...
        df = pd.DataFrame(data=data)
        df.to_csv(os.path.join(tmpdirname, 'data.csv'), index=False)
        return os.path.join(tmpdirname, 'data.csv')

    def test_filter_single_key_unwrapped(self):
        dataset = fe.dataset.CSVDataset(file_path=self._make_csv(), include_if={'mode': 'train'})
    ...
    def test_filter_single_key_multi_val(self):
        dataset = fe.dataset.CSVDataset(file_path=self._make_csv(), include_if={'mode': ('train', 'eval')})
    ...
    def test_filter_multi_key(self):
        dataset = fe.dataset.CSVDataset(file_path=self._make_csv(), include_if={'mode': 'train', 'type': 0})
        self.assertSetEqual(set(dataset['idx']), {0})
        self.assertSetEqual(set(dataset['mode']), {'train'})

    def test_filter_multi_key_multival(self):
        dataset = fe.dataset.CSVDataset(file_path=self._make_csv(), include_if={'mode': 'train', 'type': {0, 1}})
        self.assertSetEqual(set(dataset['idx']), {0, 6})
        self.assertSetEqual(set(dataset['mode']), {'train'})

    def test_filter_with_raw_None(self):
        dataset = fe.dataset.CSVDataset(file_path=self._make_csv(),
    ...
    def test_filter_function_bad_col(self):
        with self.assertRaises(ValueError):
            fe.dataset.CSVDataset(file_path=self._make_csv(),
